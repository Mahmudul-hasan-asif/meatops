...
try{
  ...
  $fields = [
    'facility_id'    => isset($d['facility_id']) ? (int)$d['facility_id'] : null,
    'location_bin'   => isset($d['location_bin']) ? trim($d['location_bin']) : null,
    'qty_on_hand_kg' => isset($d['qty_on_hand_kg']) ? (float)$d['qty_on_hand_kg'] : null,
    'qty_reserved_kg'=> isset($d['qty_reserved_kg']) ? (float)$d['qty_reserved_kg'] : null,
    'expiry_date'    => isset($d['expiry_date']) ? trim($d['expiry_date']) : null,
    'status'         => isset($d['status']) ? trim($d['status']) : null,
    'lot_code'       => isset($d['lot_code']) ? trim($d['lot_code']) : null,
  ];
  $sets=[]; $params=[':id'=>$id];
  foreach($fields as $k=>$v){ if($v!==null){ $sets[]="`$k` = :$k"; $params[":$k"]=$v; } }
  if(!$sets){ echo json_encode(['ok'=>true,'data'=>null]); exit; }

  $pdo=get_pdo();
  $sql="UPDATE batch_facility SET ".implode(',',$sets)." WHERE id=:id";
